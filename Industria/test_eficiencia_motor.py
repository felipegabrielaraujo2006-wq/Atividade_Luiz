...
from eficienci_motor import *
def test_eficiencia_motor():
    assert calcular_eficiencia(900, 1000) == 90.0
    assert (calcular_eficiencia(900, 1000) * 100) % 1 == 0
    assert calcular_eficiencia(855, 1000) == 85.5
    assert (calcular_eficiencia(855, 1000) * 100) % 1 == 0
...
def test_analise_motor():
    ...
    assert resultado["eficiencia"] == 88
